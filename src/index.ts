...
class AnkiServer {
	...
	private getFieldNames(type: "Basic" | "Cloze"): {
		front: string;
		back: string;
	} {
	...
	private setupToolHandlers() {
		...
		this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
			...
			switch (request.params.name) {
						...
						content: [
							{
								type: "text",
								text: JSON.stringify(
									await this.invokeAnki("deckNames"),
									null,
									2,
								),
							},
				...
				case "create_deck": {
					const args = validateArgs<CreateDeckArgs>(request.params.arguments, [
						"name",
					]);
				...
				case "create_note": {
					const args = validateArgs<CreateNoteArgs>(request.params.arguments, [
						"type",
						"deck",
					]);
					const { type, deck, front, back, text, backExtra, fields, tags } =
						args;
					let note;
				...
				case "batch_create_notes": {
					const args = validateArgs<BatchCreateNotesArgs>(
				...
				case "search_notes": {
					const args = validateArgs<SearchNotesArgs>(request.params.arguments, [
						"query",
					]);
				...
				case "get_note_info": {
					const args = validateArgs<GetNoteInfoArgs>(request.params.arguments, [
						"noteId",
					]);
				...
				case "update_note": {
					const args = validateArgs<UpdateNoteArgs>(request.params.arguments, [
						"id",
						"fields",
					]);
				...
				case "delete_note": {
					const args = validateArgs<DeleteNoteArgs>(request.params.arguments, [
						"noteId",
					]);
						...
						content: [
							{
								type: "text",
								text: JSON.stringify(
									await this.invokeAnki("modelNames"),
									null,
									2,
								),
							},
				...
				case "create_note_type": {
					const args = validateArgs<CreateNoteTypeArgs>(
						request.params.arguments,
